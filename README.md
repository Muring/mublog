...
- [x] 🧭 **본문 목차** — 제목 계층을 읽어 만들고, 넓은 화면에서는 옆에 떠서 현재 위치를 따라감. 좁은 화면에서는 구분선 아래 본문 첫머리에 접힌 바로 두고 펼쳐서 씀
...
- [x] 📉 **글별 조회** — 표에 발행 후 누적 조회수와 추이선. 선은 기록 시작일부터 오늘까지를 늘 같은 칸 수로 나눈다(기간이 길어지면 한 칸이 며칠). 글마다 KST 하루 1행(`post_daily_views`)
- [x] 🏷 **태그별 조회 추이** — 태그마다 선 하나씩 겹쳐 견줌. 고정 순서 색(`--series-N`), 범례로 껐다 켬, 한 칸의 값 전부를 툴팁으로
- [x] 💬 **댓글 관리** (`/admin/comments`) — 전체 댓글을 최신순으로, 상태·글·작성자 필터, 답글의 부모 인용, 삭제. "댓글" 카드와 표의 댓글 수에서 들어감
- [x] ↕️ **관리 목록 스크롤** — 포스트·댓글 목록은 공통 높이 제한 안에서 스크롤하며, 필터·개수 요약·페이지 이동은 목록 밖에 유지. 카드 모드와 로딩 화면에도 같은 경계를 적용
...
**선 색은 `--linkhovercolor`와 같은 값입니다**(라이트 `#0064dc` / 다크 `#79bbff`).
화면에서 "짚은 것"을 가리키는 색이 하나면 눈이 덜 피곤합니다. 다만 **토큰은 따로
둡니다** — 쓰임이 다르므로 한쪽을 고칠 때 다른 쪽이 조용히 따라가면 안 됩니다.

눈으로 고른 값은 아닙니다. 카드 배경 위에서 라이트 5.45:1 / 다크 7.35:1로,
비텍스트 기준 3:1을 넘습니다. 상태색(`--okcolor` 등)을 데이터 계열에 돌려쓰지
않습니다 — 그쪽은 "정상/주의"라는 뜻을 이미 갖고 있어서, 데이터에 쓰면 없는
의미가 생깁니다.
...
| `--chartbar`                            | 차트 선·면 (`--linkhovercolor`와 같은 값, 토큰은 따로) |
