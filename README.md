...
**묶는 단위로 나눕니다.** 1년치를 하루 한 칸으로 그리면 850px 안에서 한 칸이
2.3px가 되어 읽을 수도 조준할 수도 없습니다. 막대를 30개 남짓으로 붙잡습니다.
...
**Yearly는 두지 않았습니다.** 집계를 시작한 해가 하나뿐이라 막대가 1개고, 해가
바뀌어도 2개·3개입니다. 한 개짜리 막대 차트는 차트가 아니라 그냥 숫자이고,
...
0은 "아무도 안 왔다"가 아니라 "세지 않았다"라서, 그리면 없던 사실을 만듭니다.
막대 대신 점선만 남겨 자리는 지키되 값이 있는 척하지 않습니다.
...
왼쪽 y축에 0·절반·천장 세 눈금을 두고 연한 눈금선을 깔되, **값은 가장 높은 막대
하나에만 적습니다.** 서른 개에 숫자를 다 적으면 아무도 읽지 않습니다.
나머지는 호버·포커스 툴팁이 맡습니다. 눈금 천장은 1·2·5의 배수로 올려
7이나 13 같은 읽기 나쁜 수가 축에 오지 않게 합니다.

**막대 간격은 어느 단위에서나 4px입니다.** 막대 폭에 상한을 두면 개수에 따라 남는
자리가 달라져 Monthly만 벌어져 보였습니다. 간격은 격자의 `gap` 하나로만 정하고
폭은 칸을 그대로 채웁니다.
...
막대가 자라는 애니메이션은 **단위를 바꿀 때만** 돕니다. 카드가 열리는 동안에는
카드 높이도 함께 전환 중이라 두 움직임이 겹쳐 위아래로 흔들려 보입니다.
