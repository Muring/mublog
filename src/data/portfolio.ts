...
export const skills: { title: string; items: string[] }[] = [
    {
        title: "Web",
        items: [
            "HTML · CSS · JavaScript 를 프레임워크 없이도 다룰 수 있습니다. 시맨틱 마크업과 반응형 레이아웃을 기본으로 생각합니다.",
            "브라우저가 HTML 을 파싱하고 그리는 과정을 이해하고 있어, 화면이 이상할 때 프레임워크 탓을 하기 전에 DOM 과 CSS 부터 봅니다.",
            "레이아웃 계산은 JS 가 아니라 CSS(grid · calc · aspect-ratio)에 맡기는 것을 선호합니다. 렌더 전에도 무너지지 않는 화면을 만들 수 있습니다.",
            "색은 값이 아니라 토큰으로 다루고, 라이트 · 다크 두 테마를 한 벌씩 함께 설계할 수 있습니다. 대비는 감이 아니라 수치(WCAG AA)로 확인합니다.",
            "키보드로만 써 봐야 안심합니다. role · aria 속성과 포커스 흐름을 챙기는 것이 습관입니다.",
            "TypeScript 를 선호합니다. 컴포넌트 props 와 API 응답에 타입을 붙여 데이터 계약을 코드에 두는 편입니다.",
        ],
    },
    {
        title: "Vue · Nuxt",
        items: [
            "Vue 3 · Pinia · Vue Router 로 팀 프로젝트의 프론트엔드를 처음부터 세울 수 있습니다. 세 번 리드했습니다.",
            "Nuxt 3 의 페이지 · 레이아웃 · 컴포저블 구조에 익숙하고, 모바일 우선 화면도 만들 수 있습니다.",
            "Vuetify 든 Tailwind CSS 든 프로젝트 성격에 맞춰 고르고, 디자인 시안이 있을 때는 SCSS 로 직접 잡는 쪽을 선호합니다.",
            "Axios 인스턴스를 전역화하고 API 모듈을 도메인별로 나누는 구조를 첫 주에 세웁니다. 팀원이 페이지에만 집중하게 하기 위해서입니다.",
        ],
    },
    {
        title: "React · Next.js",
        items: [
            "Next.js App Router 로 서버 컴포넌트와 클라이언트 컴포넌트의 경계를 나눠 설계할 수 있습니다. 데이터 접근은 서버에, 상호작용은 클라이언트에 둡니다.",
            "SSG · ISR · 온디맨드 렌더를 상황에 맞게 섞고, 캐시 태그로 재배포 없이 내용을 갱신하는 구조를 만들 수 있습니다.",
            "서버 상태는 TanStack Query 로, 클라이언트 상태는 Zustand 로 나누는 것을 선호합니다. 낙관적 갱신과 롤백까지 구현할 수 있습니다.",
            "Emotion · Sass 같은 CSS-in-JS 와 전처리기를 모두 써 봤고, 팀의 기존 선택을 따르는 쪽입니다.",
            "알림 · 확인창 같은 공통 UI 는 컨텍스트 + 훅으로 한 곳에 두어 화면 어디서든 같은 모양이 나오게 만듭니다.",
        ],
    },
    {
        title: "Salesforce",
        items: [
            ...
            "표준 객체와 커스텀 객체를 설계하고, Path · 레이아웃 · 자동화까지 영업 프로세스를 한 화면에 얹을 수 있습니다.",
            "Experience Cloud 로 외부에 노출되는 페이지를 만들 수 있고, LWC 로 표준 컴포넌트가 못 하는 화면을 채웁니다.",
            "SFDX · Git 기반 CI/CD 를 구성할 수 있습니다. 메타데이터를 손으로 옮기는 것보다 파이프라인에 태우는 것을 선호합니다.",
            "Agentforce · Data Cloud 로 AI 챗봇을 만들어 봤습니다. 새 기능이 나오면 먼저 써 보는 편입니다.",
        ],
    },
    {
        title: "Backend · Infra",
        items: [
            "PostgreSQL(Supabase) + Prisma 로 스키마부터 API 까지 혼자 세울 수 있습니다. RLS · 트리거 · 인덱스는 raw SQL 로 다룹니다.",
            "무료 티어처럼 제약이 있는 환경을 좋아합니다. 제약이 스키마와 집계 방식을 결정하게 두는 편입니다.",
            "Docker 로 서비스를 띄우고 클라우드 VM 에 배포할 수 있습니다. Compose 파일로 환경을 문서화하는 것을 선호합니다.",
            "TypeORM 으로 풀스택 프로젝트를 설계 · 구현한 경험이 있어, 프론트만 보지 않고 API 계약부터 함께 잡습니다.",
            "보안은 나중에 붙이지 않습니다. 권한 판별의 단일 진실, 노출되는 키, 익명 접근 경로를 설계 단계에서 확인합니다.",
        ],
    },
    {
        title: "Design",
        items: [
            "Figma 로 화면을 먼저 그리고 시작합니다. 디자이너가 없는 팀에서 디자인을 맡을 수 있습니다.",
            "픽셀아트로 캐릭터 · 배경 · 아이콘과 애니메이션을 직접 만들 수 있습니다.",
            "디자인 시스템을 토큰과 조각(버튼 · 면 · 말줄임)으로 나눠 코드에 옮기는 것을 선호합니다. 같은 것이 두 곳에 다르게 생기지 않게 합니다.",
            "\"예뻐 보이는가\" 보다 \"읽히는가\" 를 먼저 봅니다. 대비 · 줄 길이 · 여백을 수치로 정합니다.",
        ],
    },
    {
        title: "ETC",
        items: [
            "Git 으로 협업하며 브랜치 전략과 커밋 단위를 지키려고 노력합니다. 커밋 메시지에 \"왜\" 를 남깁니다.",
            "Jira · Notion · Slack 으로 일정과 명세를 관리하는 데 익숙합니다. 백엔드와 API 명세를 먼저 맞춘 뒤 화면을 붙입니다.",
            "WSL2 · Linux 환경에서 개발하는 것을 선호하고 CLI 에 익숙합니다. 반복 작업은 스크립트로 만들어 둡니다.",
            "고친 것을 글로 남깁니다. 저장소 README 에 구조와 함께 그렇게 정한 이유를 적고, 배운 것은 블로그에 씁니다.",
            "리더를 맡으면 먼저 길을 닦습니다. 초기 환경과 공통 규칙을 세워 팀원이 각자의 몫에만 집중하게 합니다.",
        ],
    },
];
