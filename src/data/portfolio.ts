...
export const skills: { title: string; items: string[] }[] = [
    {
        title: "Web",
        items: [
            "브라우저가 HTML 을 파싱하고 그리는 과정을 이해하고 있어, 서버 HTML 과 DOM 이 갈리는 하이드레이션 오류를 원인부터 찾습니다.",
            "CSS 변수로 테마 토큰을 한 벌씩 두고 라이트 · 다크를 갈라 씁니다. 본문 글자는 두 테마 모두 WCAG AA(4.5:1)를 넘기도록 맞춥니다.",
            "폭 계산은 CSS(calc · grid auto-fill)에 맡기고 JS 는 개수만 정하는 식으로, 하이드레이션 전에도 레이아웃이 무너지지 않게 합니다.",
            "sitemap · canonical · Open Graph · Search Console 로 검색 노출을 관리하고, next-pwa 로 PWA 를 적용해 봤습니다.",
            "role · aria-* 속성과 포커스 관리(대화상자 안에 Tab 가두기, 닫을 때 되돌리기)를 챙깁니다.",
        ],
    },
    {
        title: "Vue · Nuxt",
        items: [
            "Vue 3 로 세 프로젝트(집우리 · SEAS · 모모뱅크)의 프론트엔드를 이끌었습니다. Pinia 로 상태를 나누고 Vue Router 로 비로그인 라우팅 가드를 세웁니다.",
            "Nuxt 3 로 모바일 뱅킹 화면(모모뱅크)을 만들었습니다. 페이지 · 레이아웃 · 컴포저블 구조에 익숙합니다.",
            "Vuetify 와 Tailwind CSS 를 프로젝트 성격에 맞춰 골라 썼고, 디자인 시안이 있을 때는 SCSS 로 직접 잡습니다.",
            "Axios 인스턴스를 전역화해 baseURL · 헤더 · 에러 처리를 한 곳에 두고, API 모듈을 도메인별로 나눕니다.",
        ],
    },
    {
        title: "React · Next.js",
        items: [
            "Next.js 14 로 Vita 를, Next.js 16(App Router)으로 이 블로그를 만들었습니다. 실무에서도 Next.js · TypeScript 로 두 프로젝트를 설계 · 구현했습니다.",
            "서버 컴포넌트와 클라이언트 컴포넌트의 경계를 나누고, 데이터 접근 계층을 서버에 두어 page → props → 클라이언트 흐름을 지킵니다.",
            "ISR 과 revalidateTag 로 재배포 없이 내용을 갱신하고, generateStaticParams 로 빌드 시점 프리렌더와 온디맨드 렌더를 함께 씁니다.",
            "TanStack Query 로 서버 상태를 다루고, cancelQueries → 스냅샷 → 낙관적 갱신 → 롤백의 뼈대로 댓글 같은 상호작용을 만듭니다.",
            "Zustand 와 Emotion(styled)을 쓰며, 컨텍스트 프로바이더 + 훅(useToast · useConfirm)으로 화면 어디서든 같은 알림 · 확인창을 띄웁니다.",
        ],
    },
    {
        title: "Salesforce",
        items: [
            "Sales Cloud 위에서 LWC · Apex · Flow · Validation Rule 을 함께 씁니다. 코드로 풀 것과 선언적으로 풀 것을 나눠 유지보수가 쉬운 쪽을 고릅니다.",
            "LWC 의 @wire · getRecord/getRecordUi · 파일(ContentDocument) 처리처럼 실무에서 부딪힌 것들을 블로그에 정리해 두었습니다.",
            "Experience Cloud 로 외부에 노출되는 Web-to-Lead 페이지를 만들었고, Static Resource 로 이미지 자산을 관리했습니다.",
            "SFDX · Git 기반 CI/CD 파이프라인을 구성해 본 경험을 기본 · 심화 · 실습 세 편으로 정리했습니다.",
            "Agentforce · Data Cloud 로 B2B 고객 지원 AI 챗봇을 만들어 해커톤에서 우승했습니다 🏆",
        ],
    },
    {
        title: "Backend · Infra",
        items: [
            "Supabase(PostgreSQL) + Prisma 로 이 블로그의 백엔드를 설계했습니다. RLS · 트리거 · 인덱스를 raw SQL 마이그레이션으로 관리합니다.",
            "무료 티어(Supabase Free · Vercel Hobby)에서 돌아가도록 커넥션 풀링, 하루 1행 집계, 캐시 태그 같은 제약을 설계에 반영했습니다.",
            "Docker 로 DB 와 애플리케이션을 띄우고 GCP VM 에 배포하는 과정을 여덟 편의 글로 정리했습니다.",
            "실무에서 TypeORM 으로 풀스택 프로젝트를 설계 · 구현했습니다.",
        ],
    },
    {
        title: "Design",
        items: [
            "Figma 로 화면을 먼저 그리고 시작합니다. 다섯 프로젝트 모두 페이지 디자인을 직접 잡았습니다.",
            "캐릭터 · 배경 · 아이콘과 걷기 · 덤벨 애니메이션을 픽셀아트로 직접 만들어 봤습니다 (Vita).",
            "색을 토큰으로만 쓰고 대비를 수치로 확인합니다. 감으로 고른 색이 다크 모드에서 사라지는 것을 여러 번 겪은 뒤의 습관입니다.",
            "Shopify 테마를 커스터마이징하고 섹션을 만들어 본 경험이 있습니다.",
        ],
    },
    {
        title: "ETC",
        items: [
            "Git · GitLab · GitHub 으로 협업하고, Jira · Notion · Slack 으로 일정과 명세를 관리합니다.",
            "6인 팀에서 프론트엔드 리더를 세 번, 4인 팀에서 PL 을 한 번 맡았습니다. 백엔드와 API 명세를 먼저 맞춘 뒤 화면을 붙입니다.",
            "WSL2 · Ubuntu 환경에서 개발하며 CLI 에 익숙합니다. 반복 작업은 스크립트로 만들어 둡니다.",
            "고친 것을 글로 남깁니다. 저장소 README 에는 무엇이 어떻게 돼 있는지와 그렇게 정한 이유를 함께 적어 둡니다.",
        ],
    },
];
